...
import (
	"net/http"
	"tradething/app/bn/future/BFF/position_mangement/handler/req"
	"tradething/app/bn/future/BFF/position_mangement/handler/res"
...
func (h *getHandler) Handler(c echo.Context) error {
	...
	position, err := h.service.Get(c.Request().Context(), reqBody.ToDomain())
	if err != nil {
	...
	responseData = responseData.FromDomain(position)
